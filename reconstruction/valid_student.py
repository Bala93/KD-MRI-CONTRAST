...
def create_data_loaders(args):
    ...
    data = SliceData(args.data_path,args.acceleration_factor,'validation', args.contrast_type)
...
def run_model(args, model,data_loader):
    ...
    with torch.no_grad():
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            input, input_kspace, target, fnames, slices  = data # Return kspace also we can ignore that for train and test 
            ...
            for i in range(recons.shape[0]):
                reconstructions[fnames[i]].append((slices[i].numpy(), recons[i][0].numpy()))
