...
from models import DCStudentNet
import torchvision
...
def create_datasets(args):
    ...
    train_data = SliceData(args.dataset_path, args.acceleration_factor, 'train')
    dev_data = SliceData(args.dataset_path,args.acceleration_factor, 'validation')
...
def train_epoch(args, epoch,model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        input, input_kspace, target, _, _, context = data # Return kspace also we can ignore that for train and test 
        ...
        context = context.float().unsqueeze(1).to(args.device)

        output = model(input, input_kspace, context)
...
def evaluate(args, epoch, model,data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            input, input_kspace, target, _, _, context  = data 
...
def visualize(args, epoch, model,data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            output = model(input,input_kspace, context)[-1]

            save_image(input[:,0,:,:].unsqueeze(1),'Input')

            save_image(target[:,0,:,:].unsqueeze(1), 'Target')

            save_image(output[:,0,:,:].unsqueeze(1), 'Reconstruction')
            save_image(torch.abs(target[:,0,:,:].unsqueeze(1).float() - output[:,0,:,:].unsqueeze(1).float()), 'Reconstruction error')
